...
def test_index(client:testing.FlaskClient) -> None:
    """Tests that the user list displays correctly."""
...
def test_edit(client:testing.FlaskClient) -> None:
    ...
    update_data_duplicate = {'csrf_token': flask.g.csrf_token, 'email': user1['email']}
    response_duplicate = client.post('/users/1/edit', data=update_data_duplicate)
