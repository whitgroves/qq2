...
def get_user(_id):
    ...
    user = ext.db.session.get(models.User, _id)
    if not user:
        return flask.redirect(flask.url_for('users.index'))
    return flask.render_template('users/user.html', user=user)

@bp.route('/<int:_id>/edit', methods=('GET', 'POST'))
@flask_login.login_required
def edit(_id):
    """Handles user profile updates."""
    if _id != flask_login.current_user.id:
        flask.abort(403)
    user = ext.db.session.get(models.User, _id)
    if not user:
    ...
    match flask.request.method:
        ...
        case 'POST':
            ...
            errors = False
            if len(models.User.query.filter_by(email=email).all()) > int(user.email == email):
            ...
            if not errors and form.validate_on_submit():
                ...
                return flask.redirect(flask.url_for('users.edit', _id=user.id))
            return flask.render_template('users/edit.html', user=user, form=form), 400
        case _:
            flask.abort(405)

@bp.post('/<int:_id>/delete/')
@flask_login.login_required
def delete(_id):
    """Handles user profile deletions."""
    if _id != flask_login.current_user.id:
        flask.abort(403)
    user = models.User.query.get_or_404(_id)
